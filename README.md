...
### Postgres
Отслеживание текущих транзакций `select pid,now() - query_start,state,query from pg_stat_activity where query_start < now() - interval '30 second' AND state != 'idle' order by 2 desc;`
