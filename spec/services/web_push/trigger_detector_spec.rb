...
describe WebPush::TriggerDetector do
  ...
  # For AbandonedCart
  let!(:action_1) { create(:action, shop: shop, user: user, item: item_3, rating: Actions::Cart::RATING, cart_date: 2.hours.ago, cart_count: 2) }

  # For SecondAbandonedCart
  let!(:action_2) { create(:action, shop: shop, user: user, item: item_4, rating: Actions::Cart::RATING, cart_date: 26.hours.ago, cart_count: 2) }
  ...
  # For Retention
  let!(:action_4) { create(:action, shop: shop, user: user, item: item_5, timestamp: 1.month.ago.to_i) }

  # For LowOnSupply
  let!(:action_3) { create(:action, shop: shop, user: user, item: item_1) }
  ...
  let!(:order_item_1) { create(:order_item, order: order_1, shop: shop, action: action_3, item: item_1 )}
  let!(:order_item_2) { create(:order_item, order: order_1, shop: shop, action: action_3, item: item_2 )}
  let!(:order_item_3) { create(:order_item, order: order_1, shop: shop, action: action_3, item: item_4 )}
  let!(:order_item_4) { create(:order_item, order: order_2, shop: shop, action: action_3, item: item_1 )}
  let!(:order_item_5) { create(:order_item, order: order_2, shop: shop, action: action_3, item: item_2 )}
  let!(:order_item_6) { create(:order_item, order: order_3, shop: shop, action: action_3, item: item_4 )}
  let!(:order_item_7) { create(:order_item, order: order_4, shop: shop, action: action_3, item: item_1 )}
  let!(:order_item_8) { create(:order_item, order: order_4, shop: shop, action: action_3, item: item_4 )}
