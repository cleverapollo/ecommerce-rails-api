...
describe WebPush::Triggers::ProductAvailable do
  ...
  let!(:web_push_trigger) { create(:web_push_trigger, shop: shop, trigger_type: 'product_available', subject: 'test test test', message: 'test message for trigger', enabled: true ) }


  describe '.condition_happened?' do

    subject { WebPush::Triggers::ProductAvailable.new(client) }
  ...
  describe 'common methods' do
    ...
    it '.code' do
      expect(WebPush::Triggers::ProductAvailable.code).to eq 'ProductAvailable'
      expect(subject.code).to eq 'ProductAvailable'
    end
