...
describe SectoralAlgorythms::Cosmetic::Physiology do
  describe '.calculate_for' do
    ...
    context 'when cold' do
      ...
      subject { SectoralAlgorythms::Cosmetic::Physiology.new(user).value }
    ...
    context 'when have views ' do
      ...
      let(:male_items) { SectoralAlgorythms::Cosmetic::Physiology::PART_TYPES.map { |part_type| create(:item, shop: shop, gender:'m', skin_type: ['dry', 'normal'], part_type: part_type) } }
      let(:female_small_items) { SectoralAlgorythms::Cosmetic::Physiology::PART_TYPES.map { |part_type| create(:item, shop: shop, gender: 'f', skin_type: ['oily', 'comby'], part_type: part_type) } }

      context 'when user small view' do
        subject {
          service = SectoralAlgorythms::Service.new(user, [SectoralAlgorythms::Cosmetic::Physiology])

          2.times { service.trigger_action('view', male_items) }
          2.times { service.trigger_action('view', female_items) }


          SectoralAlgorythms::Cosmetic::Physiology.new(user).value
        }
