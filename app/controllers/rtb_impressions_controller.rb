class RtbImpressionsController < ApplicationController
  ...
  def create
    ...
    rtb_impression = RtbImpression.create! code: "popunder-#{rand}-#{Time.now.to_i}", bid_id: "random-stub-#{rand}-#{Time.now.to_i}", ad_id: rtb_job.id, price: '1.5', currency: 'rub', shop_id: @shop.id, item_id: rtb_job.item_id, user_id: rtb_job.user_id, date: Time.current, ssp: 'propeller'
