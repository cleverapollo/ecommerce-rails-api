      ...
      if session.segment.nil?
        ...
        if last['s1'] != params[:segment1] && last['s2'] != params[:segment2]
          ...
          if Rails.env == 'production'
            SlackNotifierWorker.perform_async('DS', "Segment changed for session `<https://rees46.com/admin/clients?code=#{session.code}|#{session.code}>`: `#{last['s2']}` -> `#{params[:segment2]}`. Referer: #{request.referer}.", 'https://hooks.slack.com/services/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')
          end
        end
      end
    end
