class EventsController < ApplicationController
  ...
  def push
    ...
    case extracted_params.action.to_sym
      when :cart
         Rtb::Broker.new(extracted_params.shop).notify(extracted_params.session, extracted_params.user, ClientCart.find_by(user_id: extracted_params.user.id, shop_id: extracted_params.shop.id, date: Date.current).try(:items) )
      when :purchase
