      ...
      def update_overall
        ...
        Rails.logger.warn "   - digests_overall: #{Benchmark.ms {
          if shop.double_opt_in_by_law?
            update_params[:digests_overall] = shop.shop_emails.email_confirmed.suitable_for_digest_mailings.count
          else
            update_params[:digests_overall] = shop.shop_emails.suitable_for_digest_mailings.count
          end
        }.round(2)} ms"
        Rails.logger.warn "   - triggers_overall: #{Benchmark.ms {
          if shop.double_opt_in_by_law?
            update_params[:triggers_overall] = shop.shop_emails.email_confirmed.where('triggers_enabled = true').count
          else
            update_params[:triggers_overall] = shop.shop_emails.where('triggers_enabled = true').count
          end
        }.round(2)} ms"
        Rails.logger.warn "   - with_email: #{Benchmark.ms { update_params[:with_email] = shop.shop_emails.count }.round(2)} ms"
