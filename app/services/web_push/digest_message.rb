class WebPush::DigestMessage
  ...
  def initialize(client, digest, batch, safari_pusher = nil, test = false)
    ...
    if !test && ((@shop.web_push_balance < 1 && !@shop.subscription_plans.find_by(product: 'digest.webpush')) || (@shop.subscription_plans.find_by(product: 'digest.webpush') && !@shop.subscription_plans.find_by(product: 'digest.webpush').paid?))
