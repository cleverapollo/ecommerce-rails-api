class WebPush::TriggerMessage
  ...
  def initialize(trigger, client, safari_pusher = nil, test = false)
    ...
    if !test && ((@shop.web_push_balance < 1 && !@shop.subscription_plans.find_by(product: 'trigger.webpush')) || (@shop.subscription_plans.find_by(product: 'trigger.webpush') && !@shop.subscription_plans.find_by(product: 'trigger.webpush').paid?))
