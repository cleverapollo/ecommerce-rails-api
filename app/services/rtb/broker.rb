  ...
  class Broker
    ...
    def notify(user, items)
      ...
      items.each do |item|
        ...
        if item.price && item.price >= 2000 && item.widgetable? && item.is_available? && item.name.to_s.length < 255 && item.image_url.to_s.length < 255 && item.url.to_s.length < 255
          if rtb_item = RtbJob.find_by(shop_id: shop.id, user_id: user.id, item_id: item.id)
            ...
            RtbJob.create! shop_id: shop.id, user_id: user.id, item_id: item.id, date: Date.current, counter: 0, price: item.price, image: item.image_url, name: item.name, currency: shop.currency, url: item.url, logo: shop.fetch_logo_url
          end
        end
      end
    end
