class ShopKPI
  ...
  def calculate_products
    ...
    Slavery.on_slave do
      ...
      shop_metric.top_products = OrderItem.where(shop_id: shop.id, order_id: Order.where(shop_id: shop.id).where('date >= ?', 2.weeks.ago)).group(:item_id).count.to_a.sort_by { |x| x[1] }.reverse[0..9].map { |x| item = Item.find(x[0]); {id: item.id, name: item.name.present? ? item.name : item.uniqid, url: item.url, amount: x[1]} }
    end
