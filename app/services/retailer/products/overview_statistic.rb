  ...
  module Products
    class OverviewStatistic
      ...
      def initialize(shop, extended = false)
        ...
        self.industrial = Item.where(shop_id: @shop.id).where('is_fashion IS TRUE OR is_child IS TRUE OR is_cosmetic IS TRUE OR is_fmcg IS TRUE OR is_auto IS TRUE').recommendable.count
