class BrandCampaign < MasterTable
  ...
  def first_in_selection(item_ids, discount = false)
    relation = Item.recommendable.widgetable.where(id:item_ids, brand:downcase_brand).where.not(brand:nil).where('price >= ?', product_minimum_price).by_sales_rate
  ...
  def first_in_categories(shop_id, categories, excluded_ids=[], discount = false, strict_categories = false)
    relation = Item.recommendable.widgetable.in_categories(categories, any: !strict_categories ).where(shop_id:shop_id, brand:downcase_brand).where.not(id: excluded_ids, brand:nil).where('price >= ?', product_minimum_price).by_sales_rate
    relation = relation.discount if discount
    relation.limit(1)[0].try(:id)
  end
  ...
  def first_in_shop(shop_id, excluded_ids=[], discount = false)
    relation = Item.recommendable.widgetable.where(shop_id:shop_id, brand:downcase_brand).where.not(id:excluded_ids, brand:nil).where('price >= ?', product_minimum_price).by_sales_rate
    relation = relation.discount if discount
    relation.limit(1)[0].try(:id)
  end
