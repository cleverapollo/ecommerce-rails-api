class Advertiser < ActiveRecord::Base
  ...
  def first_in_categories(shop_id, categories)
    Item.in_categories(categories, any:true).where(shop_id:shop_id, brand:downcase_brand).where.not(brand:nil).first.try(:id)
  end

  def first_in_shop(shop_id)
    Item.where(shop_id:shop_id, brand:downcase_brand).where.not(brand:nil).first.try(:id)
  end
end
