...
set :linked_dirs, %w(tmp/sockets tmp/ymls log tmp/pids tmp/cache vendor/bundle tmp/optivo_mytoys nginx_mapping tmp/safari_keys resized-images)
...
namespace :sidekiq do
  task :start do
    on roles(:app), in: :sequence, wait: 5 do
      # execute "sudo /etc/init.d/sidekiq.api.rees46 start"
      execute "sudo /bin/systemctl start sidekiq.api.rees46"
    end
  end
  task :stop do
    on roles(:app), in: :sequence, wait: 5 do
      # execute "sudo /etc/init.d/sidekiq.api.rees46 stop"
      execute "sudo /bin/systemctl stop sidekiq.api.rees46"
    end
  end
  task :restart do
    on roles(:app), in: :sequence, wait: 5 do
      # execute "sudo /etc/init.d/sidekiq.api.rees46 stop"
      # execute "sudo /etc/init.d/sidekiq.api.rees46 start"
      execute "sudo /bin/systemctl restart sidekiq.api.rees46"
    end
  end
end
