class CreateInheritsUserTable < ActiveRecord::Migration
  ...
  def up
    ...
    execute <<-SQL
      ...
      CREATE USER MAPPING FOR #{config_master['username']} SERVER master_server OPTIONS (user '#{config_master["username"]}', password '#{config_master["password"]}');
  ...
  def down
    ...
    execute <<-SQL
      ...
      DROP USER MAPPING FOR #{config_master['username']} SERVER master_server;
